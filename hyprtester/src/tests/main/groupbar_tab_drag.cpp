...
struct SBarLayout {
    ...
    std::string_view name     = "horizontal";
};
...
TEST_CASE(groupbar_tab_drag) {
    ...
    NLog::log("{}Movement below the threshold must not reorder", Colors::YELLOW);
    {
        const auto BEFORE = classInSlot(HORIZONTAL, 1);
        const auto FROM   = tabMid(HORIZONTAL, 1);

        // under TAB_DRAG_THRESHOLD (4px) in the implementation, so this stays a click
        moveCursor(FROM);
        button(true);
        for (int i = 1; i <= 3; ++i) {
            moveCursor(SPoint{.x = FROM.x + i, .y = FROM.y});
            std::this_thread::sleep_for(std::chrono::milliseconds(20));
        }
        button(false);
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
